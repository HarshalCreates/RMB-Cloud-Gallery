...
class Eichercurtains extends StatefulWidget {
  @override
  _EichercurtainsState createState() => _EichercurtainsState();
}

class _EichercurtainsState extends State<Eichercurtains> {
  ...
  Future<void> fetchImageUrls() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('curtains').get();
    ...
    if (pickedFiles != null && pickedFiles.isNotEmpty) {
      try {
        for (var pickedFile in pickedFiles) {
          ...
          ).ref().child('curtains/$fileName');
          UploadTask uploadTask = storageRef.putFile(imageFile);
          TaskSnapshot snapshot = await uploadTask;

          String downloadUrl = await snapshot.ref.getDownloadURL();
          await FirebaseFirestore.instance.collection('curtains').add({'url': downloadUrl});
  ...
  Future<void> deleteImage(String url) async {
    try {
      ...
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('curtains').where('url', isEqualTo: url).get();
      if (snapshot.docs.isNotEmpty) {
        String docId = snapshot.docs.first.id;

        // Delete from Firestore
        await FirebaseFirestore.instance.collection('curtains').doc(docId).delete();
