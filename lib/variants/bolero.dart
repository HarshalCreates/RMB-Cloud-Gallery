...
class _BoleroState extends State<Bolero> {
  ...
  Future<void> fetchImageUrls() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('bolero').get();
    ...
    if (pickedFiles != null && pickedFiles.isNotEmpty) {
      try {
        for (var pickedFile in pickedFiles) {
          ...
          ).ref().child('bolero/$fileName');
          UploadTask uploadTask = storageRef.putFile(imageFile);
          TaskSnapshot snapshot = await uploadTask;

          String downloadUrl = await snapshot.ref.getDownloadURL();
          await FirebaseFirestore.instance.collection('bolero').add({'url': downloadUrl});
  ...
  Future<void> deleteImage(String url) async {
    try {
      ...
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('bolero').where('url', isEqualTo: url).get();
      if (snapshot.docs.isNotEmpty) {
        String docId = snapshot.docs.first.id;

        // Delete from Firestore
        await FirebaseFirestore.instance.collection('bolero').doc(docId).delete();
