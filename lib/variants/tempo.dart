...
class _TempoState extends State<Tempo> {
  ...
  Future<void> fetchImageUrls() async {
    try {
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('tempo').get();
    ...
    if (pickedFiles != null && pickedFiles.isNotEmpty) {
      try {
        for (var pickedFile in pickedFiles) {
          ...
          ).ref().child('tempo/$fileName');
          UploadTask uploadTask = storageRef.putFile(imageFile);
          TaskSnapshot snapshot = await uploadTask;

          String downloadUrl = await snapshot.ref.getDownloadURL();
          await FirebaseFirestore.instance.collection('tempo').add({'url': downloadUrl});
  ...
  Future<void> deleteImage(String url) async {
    try {
      ...
      QuerySnapshot snapshot = await FirebaseFirestore.instance.collection('tempo').where('url', isEqualTo: url).get();
      if (snapshot.docs.isNotEmpty) {
        String docId = snapshot.docs.first.id;

        // Delete from Firestore
        await FirebaseFirestore.instance.collection('tempo').doc(docId).delete();
